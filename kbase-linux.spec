...
# kbase-linux.spec
# PyInstaller spec for building KBase on Linux/macOS (no pythonnet / clr / .ico).
# Mirror of kbase.spec (Windows) with the platform-specific bits removed.
...
binaries = []
hiddenimports = [
    'kb.utils_yaml', 'kb.llm_config', 'kb.db_api', 'kb.db_index',
    'kb.document_info', 'kb.translate', 'kb.calibrate', 'kb.library_chat',
    'kb.engines', 'kb.engines.marker', 'kb.engines.docmind', 'kb.engines.docparser',
    'kb.serve',
    'webview', 'fitz',
]
...
exe = EXE(
    ...
    # Linux/macOS use PNG icon, not .ico
    icon=['kb/assets/kbase-logo.png'] if False else None,  # set to icon path if you ship one
)
coll = COLLECT(
    exe,
    a.binaries,
